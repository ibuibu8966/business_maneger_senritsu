...
import { Paperclip, FileText, Link2, Repeat, X, CalendarDays } from "lucide-react"
import {
  TASK_STATUS_CONFIG,
  PRIORITY_CONFIG,
  TOOL_CONFIG,
...
export function TaskRowExpanded({
  ...
  return (
    <div
      className="bg-blue-50/30 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 border-l-[3px] border-l-blue-500 mt-1 mb-2 rounded-md ml-3"
    ...
    >
        ...
        <div className="px-3 pb-3 space-y-2">
          {task.scheduleEvents && task.scheduleEvents.length > 0 && (
            <div className="space-y-1">
              {task.scheduleEvents.map((se: { id: string; title: string; startAt: string; allDay?: boolean }) => (
      ...
      {/* フッター: 作成者・作成日時・削除ボタン */}
      <div className="px-3 py-2 flex justify-between items-center bg-muted/20 rounded-b-md">
